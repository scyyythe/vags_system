...
export function Login({ onSectionChange }: { onSectionChange: (section: string) => void }) {
    ...
    <AuthLayout onSectionChange={onSectionChange} statueOnRight={false}>
      <div className="max-w-lg mx-auto w-full">
        <div className="absolute top-10 right-4 md:right-20 p-7 text-right">
          ...
          <Link to="/signup" className="text-customRed font-medium hover:text-customRed">
            Sign Up ↑
          </Link>
        </div>
        <h1 className="text-5xl font-bold text-black mb-10 text-center">Welcome Back!</h1>

        <div className="flex space-x-10 mb-8 ml-[-40px] justify-center w-[600px]">
          <Button variant="google" className="w-[290px] text-white hover:bg-red-700 shadow-[0_5px_4px_0_rgba(0,0,0,0.3)]">
            <img src="/google.svg" alt="Google" className="w-5 h-5 mr-2" />
            with Google
          </Button>
          <Button variant="facebook" className="w-[290px] shadow-[0_5px_4px_0_rgba(0,0,0,0.3)]">
        ...
        <form className="space-y-6 ml-[-40px]" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between text-sm w-[600px]">
            ...
            <a href="#" className="text-customRed hover:text-red-600">Forgot password?</a>
          </div>
          <Button type="submit" className="w-[600px] text-xl">Sign In</Button>
        </form>
